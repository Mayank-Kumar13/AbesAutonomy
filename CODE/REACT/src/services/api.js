 ...
 */
const getToken = () => localStorage.getItem('abes_token');

/**
 * Set auth token in localStorage.
 */
export const setToken = (token) => localStorage.setItem('abes_token', token);

/**
 * Remove auth token from localStorage.
 */
export const removeToken = () => localStorage.removeItem('abes_token');
...
export const notesApi = {
  ...
  async delete(id, deleteFile = true) {
    return request(`/notes/${id}?deleteFile=${deleteFile}`, {
      method: 'DELETE',
    });
  },

  async incrementView(id) {
    return request(`/notes/${id}/view`, { method: 'POST' });
  },
};
...
export const metaApi = {
  async getSubjects(filters = {}) {
    ...
    const qs = params.toString();
    return request(`/meta/subjects${qs ? '?' + qs : ''}`);
  },
